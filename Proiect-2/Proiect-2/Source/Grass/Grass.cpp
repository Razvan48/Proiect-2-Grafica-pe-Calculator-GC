...
Blade Grass::generateBlade(const glm::vec3& position)
{
    ...
    return Blade(
        glm::vec4(position.x, position.y, position.z, orientation_dis(gen)),
        glm::vec4(position.x, position.y + blade_height, position.z, blade_height),
        glm::vec4(position.x, position.y + blade_height, position.z, 0.1f),
        glm::vec4(0, position.y + blade_height, 0, 0.7f + dis(gen) * 0.3f)
    );
}
